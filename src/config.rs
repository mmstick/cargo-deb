...
impl Config {
    ...
    pub fn path_in_build(&self, rel_path: &str) -> PathBuf {
    ...
    pub fn path_in_deb(&self, rel_path: &str) -> PathBuf {
...
impl Cargo {
    fn to_config(mut self, manifest_path: &Path) -> CDResult<(Config, Vec<String>)> {
        ...
        let assets = self.take_assets(&target_dir, deb.assets.take(), readme)?;
        Ok((Config {
            target_dir,
            ...
            extended_description: self.extended_description(deb.extended_description.as_ref().map(|s|s.as_ref()), readme)?,
    ...
    fn extended_description(&self, desc: Option<&str>, readme: Option<&String>) -> CDResult<Vec<String>> {
    ...
    fn take_assets(&self, target_dir: &Path, assets: Option<Vec<Vec<String>>>, readme: Option<&String>) -> CDResult<Vec<Asset>> {
        Ok(if let Some(assets) = assets {
            assets.into_iter().map(|mut v| {
                let mut v = v.drain(..);
                Ok(Asset {
                    source_file: PathBuf::from(v.next().ok_or("missing path for asset")?),
                    target_path: PathBuf::from(v.next().ok_or("missing target for asset")?),
            ...
            let mut implied_assets: Vec<_> = self.bin.as_ref().unwrap_or(&vec![])
                ...
                .map(|bin| {
                Asset::new(
                    target_dir.join("release").join(&bin.name),
